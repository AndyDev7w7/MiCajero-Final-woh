...
@RequestMapping("/cajero") // Aquí empieza el viaje del cliente normal 😎
public class ControladorCajero {

    private final ServicioCliente servicioCliente;
    private final ServicioCuenta servicioCuenta;
    private final RepositorioCuenta cuentaRepo;
    private final ServicioMovimiento movimientoService;
    private final ServicioRetiro retiroService;
    ...
    public String login(@RequestParam String numeroCuenta,
        ...
        if (cliente.isBloqueado()) {
            model.addAttribute("error", "Tu cuenta está bloqueada, bro 😬.");
            return "cajero/login";
        }
        ...
        if (!cliente.getPin().equals(pin)) {
            ...
            if (cliente.getIntentos() >= 3) {
                ...
                model.addAttribute("error", "Bloqueado por manquear el PIN 3 veces 😵.");
            } else {
                model.addAttribute("error", "PIN incorrecto. ¿Estás bien?");
            }
            return "cajero/login";
        }
    ...
    public String movimientos(@PathVariable String numero, Model model, HttpSession session) {
        ...
        try {
            ...
            model.addAttribute("error", "No se pudo obtener movimientos: " + e.getMessage());
            return "cajero/consultas";
        }
    }
    ...
    public String consignar(@RequestParam String numeroCuenta,
                            ...
                            Model model) {
        try {
            Cuenta cuenta = cuentaRepo.findByNumero(numeroCuenta)
    ...
    public String transferir(@RequestParam String numeroCuentaDestino,
        ...
        Cuenta origen = servicioCuenta.buscarPorCliente(cliente)
                .stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No se encontró cuenta de origen"));
        ...
        try {
            ...
            if (movimientoService.realizarTransferencia(origen, destino, monto)) {
                model.addAttribute("mensaje", "Transferencia completada ✅");
            } else {
                model.addAttribute("error", "Saldo insuficiente para la transfer.");
            }
        } catch (Exception e) {
    ...
    public String cambiarClave(@RequestParam String claveActual,
        ...
        session.setAttribute("cliente", cliente);
        model.addAttribute("mensaje", "PIN actualizado con éxito 🎉");
